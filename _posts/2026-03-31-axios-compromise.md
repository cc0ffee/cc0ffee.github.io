...
- Windows (ps1, exe, vbs):`617b67a8e1210e4fc87c92d1d1da45a2f311c08d26e89b12307cf583c900d101`
- macOS:`92ff08773995ebc8d55ec4b8e1a225d0d1e51efa4ef88b8849d0071230c9645a`
- Linux (Python):`fcb81618bb15edfdedfb638b4c08a2af9cac9ecfa551af135a8402bf980375cf`
...
The versions were available for download for 2-3 hours until it was removed from NPM. The maintainer states that they had MFA on everything, and how it was bypassed is unknown as of now _(perhaps fatigue attacks, interception?)_
## Part 1: Dropper
...
3. Writes a VBScript (`.vbs`) file that uses `cmd.exe` to curl a PowerShell script (`.ps1`) from the C2 server with POST Body `packages.npm[.]org/product1`, and runs silent with `-w hidden -ep bypass`.
...
It is also worth noting that `packages.npm[.]org/productx` is a fake domain masquerading as npm to blend in with legitimate traffic, it is most likely resolved as something else on the C2 server.

The malicious dependency deletes itself, and changes the `package.json` file to hide its tracks.
...
- Gets information about the system: OS version, architecture, boot time, hostname, username, installation time, manufacturer/product name, and process list
